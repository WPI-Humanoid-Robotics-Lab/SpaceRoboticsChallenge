...
int main(int argc, char** argv)
{
  ...
  right_armTraj = new ArmControlInterface(nh);
  left_armTraj = new ArmControlInterface(nh);
...
void execute_callback(const std_msgs::Int16& msg)
{
  if(msg.data)
  {
    ...
    if(left_pose_received)
    {
      ...
      std::vector<ArmControlInterface::armTaskSpaceData>* arm_data_vector = generate_task_space_data(left_poses,RobotSide::LEFT);
    ...
    if(right_pose_received)
    {
      ...
      std::vector<ArmControlInterface::armTaskSpaceData>* arm_data_vector = generate_task_space_data(right_poses,RobotSide::RIGHT);
      // moveArmInTaskSpace(std::vector &arm_data, int baseForControl) baseForControl defaults to chest
...
std::vector<ArmControlInterface::armTaskSpaceData>* generate_task_space_data(std::vector<geometry_msgs::PoseStamped*>* input_poses, RobotSide input_side, float desired_time)
{
