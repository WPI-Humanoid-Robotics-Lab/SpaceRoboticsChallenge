    ...
    nh_(nh)
{
    ...
    logFile = ros::package::getPath("src_task1") + "/log/task1.csv";
...
bool task1Utils::getNextPoseToWalk(geometry_msgs::Pose2D &pose2D, bool allowVisitied)
{
    ...
    for (float theta = 0; theta < M_PI; theta += M_PI/6){
        for (int i = -1; i < 2; i+=2){
            ...
            current_state_->transformPose(pose2D, pose2D, rd_->getPelvisFrame(), TOUGH_COMMON_NAMES::WORLD_TF);
...
void task1Utils::visulatise6DPoints(std::vector<geometry_msgs::Pose> &points)
{
    ...
    for (int i = 1; i < points.size(); i++) {
        ...
        marker_tangent.header.frame_id = TOUGH_COMMON_NAMES::WORLD_TF;
        ...
        marker_point.header.frame_id = TOUGH_COMMON_NAMES::WORLD_TF;
